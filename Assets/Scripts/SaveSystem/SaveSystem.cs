...
namespace SaveSystem
{
    public static class SaveSystem
    {
        ...
        public static void SaveLevelData( int[] newEnemiesIds,int newCurrentLevel, int newBackground, int newLevelEnemiesPoints)
        {
            BinaryFormatter formatter = new BinaryFormatter();
            string path = Application.persistentDataPath + "/levelData.rotd";
            FileStream stream = new FileStream(path, FileMode.Create);
        
            LevelData data = new LevelData(newEnemiesIds,newCurrentLevel,newBackground,newLevelEnemiesPoints);
            formatter.Serialize(stream, data);
            stream.Close();
        }
